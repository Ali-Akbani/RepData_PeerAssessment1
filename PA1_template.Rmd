...
```{r echo=TRUE}
steps_per_interval[ steps_per_interval$average == max( steps_per_interval$average), "interval"];
```
...
```{r echo=TRUE}
imputed_activity_data <- merge( activity_data, steps_per_interval, by="interval", sort=FALSE);
```
...
imputed_activity_data$steps.x = ifelse( is.na(imputed_activity_data$steps.x)
																				,imputed_activity_data$average
																				,imputed_activity_data$steps.x);
```

Now to reshape the data frame in the image of the original activity_data

```{r echo=TRUE}
imputed_activity_data <- imputed_activity_data[ order(imputed_activity_data$date) ,c(2,3,1)];
...
```{r echo=TRUE}
part_of_week <- ifelse( weekdays( imputed_activity_data$date ) == "Saturday" | weekdays( imputed_activity_data$date ) == "Sunday", "Weekend","Weekday");
```
...
```{r echo=TRUE}
library(ggplot2);
qplot(x=interval,y=average,data=steps_per_interval,facets= part_of_week~., geom="line");
```
